...
car_rect = car_img.get_rect(center=(WIDTH // 2, HEIGHT - 100))
...
while running:
    ...
    if result.multi_hand_landmarks:
        for hand_landmarks in result.multi_hand_landmarks:
            ...
            # Apply smoothing to the car position based on index finger tip
            ...
            # Update car position with smoothed coordinates
            car_rect.centerx = smoothed_x
            car_rect.centery = smoothed_y
    ...
    # Move thunder and add new ones
    if random.randint(1, obstacle_frequency) == 1:
        rock_rect = thunder_img.get_rect(center=(random.randint(50, WIDTH - 50), -50))
    ...
    # Move jackpot thunder and add new ones
    if random.randint(1, 500) == 1:
        jackpot_rock_rect = jackpot_img.get_rect(center=(random.randint(50, WIDTH - 50), -50))
        jackpot_thunder.append(jackpot_rock_rect)
        jackpot_rock_timer = 0
    ...
    # Move space and add new ones
    if random.randint(1, obstacle_frequency) == 1:
        truck_rect = space_img.get_rect(center=(WIDTH + 50, random.randint(50, HEIGHT - 50)))
    ...
    # small collision
    collision_rect = car_rect.inflate(-car_rect.width * 0.3,-car_rect.height * 0.3)

    # Collision detection with thunder
    for rock_rect in thunder:
        if collision_rect.colliderect(rock_rect):
            running = False

    # Collision detection with space
    for truck_rect in space:
        if collision_rect.colliderect(truck_rect):
            running = False
    
    # Collision detection with jackpot thunder
    for jackpot_rock_rect in jackpot_thunder:
